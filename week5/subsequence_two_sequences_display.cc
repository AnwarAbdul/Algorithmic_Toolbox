...
int backtrack_edit_distance_matrix(int **a,int n1,int n2,int *s1,int *s2)
{
	...
	while(i >= 1 && j >= 1)
	{
		...
		{
			cout<<"\nSubsequence print s1 , s2 " <<s1[i-1]<<" "<<s2[j-1]<<" i = "<<i<<" j = "<<j<<"\n";
			subsequence_length++;
			i--;
			j--;
		}
...
int sequence_alignment(int *a,int *b,int n1,int n2)
{
	...
	int subsequence_length = backtrack_edit_distance_matrix(edit_distance_matrix,n1,n2,a,b);
	return subsequence_length;
}
